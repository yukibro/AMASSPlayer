...
public class VRCaptureManager : MonoBehaviour
{
    ...
    private List<Matrix4x4> calibrated_frames;
    
    void Start()
    {
        calibrated = false;    
        frames = new List<Matrix4x4>();
        calibrated_frames = new List<Matrix4x4>();
    ...
    void CollectFrame()
    {
        ...
        calibrated_frames.Add(hmd.localToWorldMatrix);
        calibrated_frames.Add(l_controller.localToWorldMatrix);
        calibrated_frames.Add(r_controller.localToWorldMatrix);
    ...
    void Save4x4()
    {
        ...
        String filePath = "./VR_recorded.txt";

        using (StreamWriter writer = new StreamWriter(filePath))
        {
            for (int f = 0; f < frames.Count; f++)
            {
                writer.Write(frames[f].ToString("F8") + ",");
            }
        }

        Debug.Log("Saving...Calibrated...");

        String filePath_2 = "./VR_recorded_calibrated.txt";

        using (StreamWriter writer = new StreamWriter(filePath_2))
        {
            for (int f = 0; f < calibrated_frames.Count; f++)
            {
                writer.Write(calibrated_frames[f].ToString("F8") + ",");
            }
        }
    }
